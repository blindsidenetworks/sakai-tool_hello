    ...
    if('currentMeetings' === state) {
        ...
        if(bbbUserPerms.bbbViewMeetingList) {
            ...
            for(var i=0,j=bbbCurrentMeetings.length;i<j;i++) {
                HelloUtils.setMeetingPermissionParams(bbbCurrentMeetings[i]);
                HelloUtils.setMeetingInfoParams(bbbCurrentMeetings[i]);
                HelloUtils.setMeetingJoinableModeParams(bbbCurrentMeetings[i]);
            }
            HelloUtils.render('bbb_rooms_template',{'meetings':bbbCurrentMeetings},'bbb_content');

            // show tool footer message only if site maintainer
            //if(bbbUserPerms.bbbAdmin) {
            if(bbbUserPerms.siteUpdate) {
            	bbbToolVersion = HelloUtils.getToolVersion();
            	HelloUtils.render('bbb_toolfooter_template',{'bbbTool':bbbToolVersion},'bbb_footer');
            }
            ...
            bbbCheckAllMeetingAvailabilityId = setInterval("HelloUtils.checkAllMeetingAvailability()", bbbInterval.meetings);
        ...
        var meeting = isNew ? {} : HelloUtils.getMeeting(arg.meetingId);
        var contextData = {
                'isNew':        isNew,
                'meeting':      meeting,
                'selTypes':     HelloUtils.getUserSelectionTypes(),
                'selOptions':   HelloUtils.getUserSelectionOptions(),
                'siteId':       bbbSiteId,
        ...
        HelloUtils.render('bbb_addUpdate_meeting_template', contextData, 'bbb_content');
        ...
        $(document).ready(function() {
            ...
            HelloUtils.makeInlineFCKEditor('bbb_welcome_message_textarea', 'Basic', '480', '200');
            ...
            jQuery("#bbb_add_update_form").submit(function() {
                HelloUtils.addUpdateMeeting();
                return false;
            });
            
            // User warnings
            if(!allSiteMembersCanParticipate()) {
                 HelloUtils.showMessage(bbb_err_not_everyone_can_participate);
            }
        ...
        $(document).ready(function() {
            $('#bbb_permissions_save_button').bind('click', function() {
               HelloUtils.setSitePermissions('.bbb_permission_checkbox', function() {
                   // success callback
                   bbbUserPerms = new BBBPermissions(HelloUtils.getUserPermissions());
                   if(bbbUserPerms.bbbViewMeetingList)
                       refreshMeetingList();
                   switchState('currentMeetings');
                   if(bbbUserPerms.bbbViewMeetingList) 
                        HelloUtils.showMessage(bbb_permissions_saved, 'success');
               })
            });
...
function allSiteMembersCanParticipate() {
    var perms = HelloUtils.getSitePermissions();
...
function addParticipantSelectionToUI(meeting, isNew) {
    var selOptions = HelloUtils.getUserSelectionOptions();
	if(isNew) {
	...
	if(meetingInfo != null) {
		if(meetingInfo.participantCount != null && parseInt(meetingInfo.participantCount) >= 0) {
			...
			jQuery('#attendees, #moderators').tipTip({
			     activation:'click', 
			     keepAlive:'true', 
			     enter:function(){ setTimeout("HelloUtils.adjustFrameHeight();",1000); }
			});
...
function refreshMeetingList() {
	bbbCurrentMeetings = HelloUtils.getMeetingList(bbbSiteId);
...
function refreshRecordingList(meetingId) {
	...
	var getRecordingResponse = (meetingId == null)? HelloUtils.getSiteRecordingList(bbbSiteId): HelloUtils.getMeetingRecordingList(meetingId);
	...
	if ( getRecordingResponse.returncode == 'SUCCESS' ){
		...
		if ( getRecordingResponse.messageKey != null ){
	    	HelloUtils.showMessage(getRecordingResponse.messageKey + ":" + getRecordingResponse.message, 'warning');
		} else {
	    	HelloUtils.showMessage("An unidentified error has just happened", 'warning');
		}
	}
